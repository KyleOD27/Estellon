#!/usr/bin/env python3
"""
Generate all character voices using Windows TTS
"""
...
def generate_character_voice(character, data):
    ...
    try:
        ...
        if Path(output_file).exists():
            ...
            print(f"✓ {character.title()}: Generated {output_file} ({file_size} bytes)")
            return True
        else:
            print(f"✗ {character.title()}: Failed to generate audio")
            return False
            
    except Exception as e:
        print(f"✗ {character.title()}: Error - {e}")
        return False
