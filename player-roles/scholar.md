# The Scholar

> *"In Estellon's libraries lie forgotten laws, ancient precedents, and the power to reshape reality through knowledge."*

## Role Overview

As a Scholar, you are a seeker of truth and wielder of knowledge, drawn to Estellon by its ancient archives and the philosophical questions its crisis poses. You understand that information is power, and in a city torn between Law, Chaos, and Order, your expertise in history, law, and philosophy makes you invaluable to all sides.

## Background Hooks

### Why You're Here
Choose one hook below. Your choice grants special dialogue opportunities and rewards specific behaviors.

#### **Academic Pilgrimage**
*Studying Estellon's famous legal archives*

**Special Dialogue Access:**
- Can request access to restricted archives from librarians
- Recognized by other scholars and archivists throughout the city
- Can ask about "ancient texts" or "historical precedents" with any educated NPC

**Behavior Rewards:**
- **Archive Privileges:** +2 to Investigation checks when researching in libraries or archives
- **Scholar's Network:** Once per session, can contact a fellow academic for specialized knowledge on any topic
- **Legal Authority:** When citing historical precedents, +1 to Persuasion with lawful-aligned NPCs

---

#### **Philosophical Tourism**
*Drawn by the city's ideological conflict*

**Special Dialogue Access:**
- Can engage nobles and faction leaders in philosophical debates
- Other intellectuals welcome discussions on Law vs Chaos themes
- Can frame political questions as "academic curiosity" without suspicion

**Behavior Rewards:**
- **Ideological Insight:** +2 to Insight checks when analyzing someone's philosophical position or motivations
- **Devil's Advocate:** Once per session, can argue the opposite of your position convincingly, gaining advantage on a Persuasion check
- **Neutral Observer:** No initial reputation penalty with any faction; seen as studying, not participating

---

#### **Historical Research**
*Investigating ancient texts and precedents*

**Special Dialogue Access:**
- Can interview elderly citizens about "the old days"
- Access to family archives of noble houses for "research purposes"
- Can ask clergy about "church historical records"

**Behavior Rewards:**
- **Historical Parallel:** Once per session, can recall a relevant historical event that provides +2 to a check or grants insight into current situation
- **Document Hunter:** +2 to Investigation when searching for specific texts, records, or evidence
- **Living History:** When revealing historical facts, can influence NPC opinions; successful History check grants +1 faction reputation

---

#### **Institutional Representative**
*Sent by universities or temples to observe*

**Special Dialogue Access:**
- Can request official audiences with faction leaders as "neutral observer"
- Access to diplomatic spaces and official functions
- Can take notes publicly without raising suspicion

**Behavior Rewards:**
- **Official Status:** +2 to Persuasion when invoking your institutional authority
- **Diplomatic Immunity:** Once per session, can claim "observer status" to avoid being forced to take sides in a situation
- **Report Back:** At the end of each session, can send a report to your institution and receive 25gp stipend + strategic advice

---

### Starting Resources
- Access to library archives
- Letters of introduction to learned societies
- Rare books and scrolls
- Network of academic contacts
- Knowledge of obscure laws and precedents

## Starting Bonuses (Mechanical)
- +2 to Investigation checks
- +2 to one knowledge skill (Arcana, History, Nature, or Religion)
- +2 Coalition Strength (Elara's faction) score
- Scholar's pack with research materials

## Social Access
- **Access to libraries, archives, and universities** - Academic credentials open scholarly doors
- **Can request audiences with learned nobles and clergy** - Intellectual reputation carries weight
- **Recognized by Ser Elara's reformist circle** - Education aligns with reform ideals
- **Can consult with scribes and record-keepers** - Fellow scholars share knowledge

## Role Abilities

Your scholarly expertise grants you these abilities:

### Active Abilities
1. **Research Network** (Once per session) - Spend downtime researching a topic and gain detailed information (legal precedents, historical context, magical theory, etc.). Your academic connections and methodical approach unlock hidden knowledge.
2. **Expert Analysis** (Once per scene) - Provide detailed insight on any knowledge-related topic, granting allies +2 to related checks for the scene.
3. **Legal Precedent** (Once per day) - Cite an obscure law or historical precedent to influence negotiations or challenge authority.

### Passive Abilities
1. **Academic Authority** - +2 to Persuasion checks when citing facts, history, or logical arguments
2. **Pattern Recognition** - Automatically notice inconsistencies in stories, documents, or events
3. **Living Library** - Can recall detailed information on subjects you've studied; never need to make "basic knowledge" checks in your fields of expertise

## Faction Perspectives

### Supporting the Reformers (Elara)
- **Legal Precedent:** Ancient laws support reform
- **Intellectual Framework:** Philosophical justification for change
- **Educational Reform:** Knowledge should serve all people
- **Institutional Wisdom:** Blend tradition with progress

### Supporting the Revolution (Kael)
- **People's Education:** Democratize knowledge and learning
- **Revolutionary Theory:** Intellectual foundation for new order
- **Historical Cycles:** Past revolutions inform present action
- **Knowledge Liberation:** Break academic gatekeeping

### Supporting the Guard (Brennis)
- **Order Through Law:** Scholarly interpretation maintains stability
- **Historical Continuity:** Preserve institutional knowledge
- **Precedent Power:** Use tradition to justify authority
- **Academic Privilege:** Current system protects scholars

## Unique Opportunities

### Intellectual Leverage
- Discover legal loopholes and precedents
- Interpret laws to favor chosen faction
- Uncover historical secrets and scandals
- Provide philosophical legitimacy

### Scholar-Specific Quests
- **The Lost Precedent:** Find ancient laws that change everything
- **Historical Revelation:** Uncover truths about Estellon's founding
- **Philosophical Debate:** Convert opponents through reason
- **Archive War:** Control or destroy key documents

### Special Assets
- **Archive Access:** Hidden knowledge in libraries
- **Academic Network:** Information from fellow scholars
- **Legal Expertise:** Understanding of complex laws
- **Historical Insight:** Context for current events

## Roleplaying Guidance

### Core Motivations
- Truth and knowledge above all
- Preserving or spreading learning
- Philosophical consistency
- Historical perspective on events

### Key Questions
- Is truth more important than stability?
- Should knowledge serve power or people?
- Can philosophy change reality?
- What does history teach about revolution?

### Scholar Phrases for Dialogue Bingo
- *"History will judge us by our choices today."*
- *"The law is clear on this matter, if properly interpreted."*
- *"Knowledge without action is merely philosophy."*
- *"The precedent for this was set centuries ago."*
- *"Truth is the first casualty of revolution."*

## Mechanical Suggestions

### Recommended Skills (System-Agnostic)
- History and research
- Investigation and analysis
- Persuasion through logic
- Arcana or religious knowledge

### Potential Backgrounds
- Sage or researcher
- Cloistered Scholar
- Noble (educated aristocrat)
- Hermit (solitary researcher)

### Starting Equipment Ideas
- Rare books and scrolls
- Writing materials
- Magnifying glass
- Scholar's robes
- Letter of academic credentials

## Campaign Integration

### Act I Opportunities
- Research faction legitimacy
- Discover festival significance
- Interpret riot causes

### Act II Developments
- Find Crown-Regent precedent
- Uncover Lord Pellor's secrets
- Legitimize faction claims

### Act III Decisions
- Document or rewrite history
- Choose what knowledge survives
- Shape the new order's foundation

## Playing a Scholar

In Estellon's crisis, you hold the power to legitimize or delegitimize any faction through your interpretation of law, precedent, and philosophy. Your research can uncover game-changing secrets, your arguments can sway nobles and commoners alike, and your knowledge of history provides crucial context for current events.

Whether you're an idealistic academic seeking truth, a pragmatic researcher trading knowledge for influence, or a revolutionary intellectual reshaping society through ideas, your scholar's journey proves that in times of upheaval, the pen truly is mightier than the sword.

### Scholar's Burden

Remember that with knowledge comes responsibility. Every fact you reveal or conceal, every law you interpret, every historical parallel you draw shapes how others understand the conflict. In Estellon, you don't just study history - you write it.

---

## Navigation

**[← Merchant](merchant.md)** | **[Sellsword →](sellsword.md)** | **[Game Overview](../game/game-overview.md)** | **[🏠 Home](../README.md)**